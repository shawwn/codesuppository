...
HRESULT DXUTFindDXSDKMediaFileCch( WCHAR* strDestPath, NxI32 cchDest, LPCWSTR strFilename )
{
	...
	mbstowcs( wmedia, media, 512 );

		
    StringCchPrintf( strDestPath, cchDest, L"%s\\%s\\%s", strExePath, wmedia, strFilename );
...
void CBaseCamera::GetInput( bool bGetKeyboardInput, bool bGetMouseInput, bool bGetGamepadInput, bool bResetCursorAfterMove )
{
    ...
    if( bGetKeyboardInput || mForceMoveForward)
    {
        // Update acceleration vector based on keyboard state
        if( IsKeyDown(m_aKeys[CAM_MOVE_FORWARD])|| mForceMoveForward )
...
HRESULT CDXUTLineManager::AddLine( NxI32* pnLineID, D3DXVECTOR2* pVertexList, DWORD dwVertexListCount, D3DCOLOR Color, NxF32 fWidth, NxF32 fScaleRatio, bool bAntiAlias )
{
    ...
    LINE_NODE* pLineNode = MEMALLOC_NEW(LINE_NODE);
    if( pLineNode == NULL )
...
HRESULT CDXUTDirectionWidget::StaticOnCreateDevice( IDirect3DDevice9* pd3dDevice )
{
    ...
    V_RETURN( D3DXCreateEffect( s_pd3dDevice, g_strBuffer, dwBufferSize, NULL, NULL, 0, NULL, &s_pEffect, NULL ) );
...
IDirect3DDevice9* DXUTCreateRefDevice( HWND hWnd, bool bNullRef )
{
    ...
    hr = pD3D->CreateDevice( D3DADAPTER_DEFAULT, bNullRef ? D3DDEVTYPE_NULLREF : D3DDEVTYPE_REF,
                             hWnd, D3DCREATE_HARDWARE_VERTEXPROCESSING | D3DCREATE_FPU_PRESERVE, &pp, &pd3dDevice );
...
NxI32 CDXUTTextHelper::GetAvgFontWidth( void )
{
	...
	if ( m_pFont )
	{
		...
		if ( m_pFont->GetTextMetrics( &textMetrics ) )
		{
