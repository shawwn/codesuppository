...
#define VMD_DEFAULT_FONT_SCALE  1.0f

//note the arbitrary scaling
inline float h2float(char c) { return VMD_DEFAULT_FONT_SCALE * (float)(c-'R'); }
...
void hersheyChar(char ch,HersheyCallback *callback)
{
	const char *cp = hersheyFontData[ch % 128];
	float lm = h2float(cp[0]);
	float rm = h2float(cp[1]);
