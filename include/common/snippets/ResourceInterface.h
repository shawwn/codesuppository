...
public:
  ...
  virtual NxI32               releaseResourceHandles(ResourceInterfaceCallback *iface) = 0; // release all resources that were registered with this callback address, returns number that were released.  Useful in destructors for guaranteed cleanup.
  virtual void                pump(void) = 0;
  virtual NxI32               getFileVersionNumber(const char* /*fqn*/,bool /*getIfMissing*/) { return -1; }; // optional method if we support file versioning.  Returns 0 if file version unavailble or -1 if this feature is not supported.  If 'getIfMissing' true, then retrive the file from the remote location.
